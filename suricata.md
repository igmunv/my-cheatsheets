...
Работает так: из одного интерфейса трафик должен пройти во второй интерфейс, но перед этим происходит проверка на DROP правила, если пакет не нарушает, то он проходит во второй интерфейс
...
- Настройка интерфейсов:
  ...
  af-packet:
    - interface: eth0
      copy-mode: ips
      copy-iface: eth1 # если все норм то отправляем на eth1
      defrag: no
      # остальные настройки по умолчанию
    - interface: eth1
      copy-mode: ips
      copy-iface: eth0 # если все норм то отправляем на eth0
      defrag: no
      # остальные настройки по умолчанию
    ```
  
  `sudo suricata -c /etc/suricata/suricata.yaml --af-packet` - запуск в режиме IPS. Используем механиз af-packet для работы с пакетами
